...
| 2b | `marching_cubes_interior` | **Interior** ambiguity resolution — MC33's sweeping plane, with Custodio's correction | Added at A-002c. The sibling of row 2: that one resolves an ambiguous *face*, this one an ambiguous *cell*. One cell with a plane sweeping through it, the bilinear saddle tracked as a dot, and its path drawn — a hyperbola, not a line, because the saddle's position is linear over `Δ(t)` and runs to infinity at `Δ`'s root (drawn magenta). The HUD prints both verdicts every frame: Chernyaev's numerator-only test against the corrected one. On configuration `1` they **disagree** — `F < 0` for the whole sweep while the saddle is positive past the pole — and on `2` they agree, which is there so the example does not misrepresent how often the failure arrives. **Not a tunnel being meshed**: that needs cell-interior vertices the grid-edge-keyed cache has no slot for, and is A-002b. |
| 2c | `marching_cubes_tunnel` | The **tunnel**, meshed as a tunnel ([gif](../gifs/the-tunnel-meshed-as-a-tunnel.gif), [still](../screenshots/e213-tunnel.png), recorded with `scripts/record_gif.sh`) | Added at E-213, and the first of the three where the *mesh* changes rather than the verdict. One cell meshed twice: left the face rule alone, giving **two separate discs**; right with the interior rule, giving **one cylinder** through the cell. The inner hexagon is drawn as a closed gold ring — six body saddles, the points Grosso's whole construction is built from — with the contours in green and magenta. Measured on the two shipped configurations: components **2 → 1**, χ **2 → 0**, boundary edges *unchanged*, which is the crack-freeness property visible as a number. **The HUD's headline is the χ difference**, because that is the claim: a tunnel is a handle and a handle costs exactly two (M-222). Configurations are hand-written and searched rather than sampled — interior ambiguity occurs in **0 of 68,385** surface cells across the seven original reference fields (M-208), so a demo that sampled a field would show an empty cell almost every time. |
