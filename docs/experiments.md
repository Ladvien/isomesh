...
**One hundred and two times**, this project wrote down what it expected to measure — and what result would
prove it wrong — **before running the measurement**. Ninety-five of those are enforced by the compiler:
`crates/isomesh/src/experiment.rs` refuses to build a harness whose id is not registered, so the prediction
