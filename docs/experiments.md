...
**One hundred and seventy-six times**, this project wrote down what it expected to measure — and what result
would prove it wrong — **before running the measurement**. One hundred and sixty-nine of those are enforced by
...
two"* and *"ninety-five"* for a whole phase after they stopped being true, and *"one hundred and
twenty-two"* and *"one hundred and fifteen"* for another after that.
...
## Phase 27 — fifty registrations from eighteen fields of mathematics, every one run

The largest phase by any measure: fifty pre-registered experiments, `P-127`…`P-176`, all fifty harnessed,
all fifty run against a clean tree, all fifty CSVs committed with clean provenance. The registrations were
transcribed from eighteen fields of mathematics before any harness existed — `isomesh::experiment!("P-127")`
was a compile error until all fifty landed — and the phase changed **nothing** in `crates/isomesh/src/**`
and moved **no golden hash**: every mechanism is bench-local, in `benches/common/`, driven through the
public API.

**Thirteen rows held on every clause, three failed on every clause they could fail, and thirty-four are
mixed.** Three registrations were nulls on purpose and came back null; three more came back *not* null,
which is why they were worth registering.

| prediction | verdict | what it means if you depend on this crate |
|---|---|---|
| **P-127** — `b*b - 4*a*c` is identically Cayley's `2×2×2` hyperdeterminant | **held**, all three (M-440) | The headline. `symbolic_difference_is_zero` **true**, 12 terms against 12 at degree 4, all **3** pencil pairings agreeing, exact-rational deviation **`0/1`** over 3,481 trials — and `f32_sign_disagreements` **14**, which is what keeps the exact-sign work alive. A shipped expression has been a classical invariant for the crate's whole life without anyone knowing |
| **P-128** — the saddle count cannot depend on cell aspect ratio | **held**, all clauses on 48 of 48 rows (M-441) | `count_disagreements`, `sign_disagreements` and `corner_value_mismatches` all **0** over **7,077,888** cells at aspect ratios 1, 4 and 8. The `GL(2)³` weight `(det g₁ det g₂ det g₃)²` is an exact `i128` equality. Group D may vary cell aspect ratio without re-asking whether interior ambiguity moved |
| **P-129** — invariance as algebra, 10× cheaper than the 48-element sweep | **falsified**, C1 and C3 (✗99 / M-442) | The `Δ`-invariance check costs one evaluation **per element**, not per cell, so `relabellings_per_delta` reads **0.980** and the bar was dead on an integer count before any clock ran. The harness's first draft compared the case word two ways and was a **tautology** — `case_of(relabel(f))` and `relabel_case(case_of(f))` are the same function by construction — which the vacuity control caught by measuring zero violations on a deliberately asymmetric tuple. The shipped sweep compares the shipped table's *triangulation* under the element instead |
| **P-130** — does rank predict ambiguity better than the case index | **falsified**, C1 and C2 (✗100 / M-443) | `delta_zero_share` reaches **1.000000** on `box_exact` — every surface cell sits *exactly* on the hyperdeterminant hypersurface. C2 is **arithmetically unreachable**: `ambiguous = g(case)` makes `I(R;A) ≤ I(R;K)` a data-processing inequality, so no measurement could have made it hold |
| **P-131** — the `Δ = 0` branch fires and is not the tangency the comment describes | **held**, all three (M-444) | **143** hits on reference fields, of **3,858** branch hits **2,222** true rank 3 with border rank 2 — de Silva & Lim's W-state class, not the tangential touch the code comment describes. The registered W-state fixture reaches the branch with `w_state_rank` **3** against `w_state_border_rank` **2** |
| **P-132** — orbit class predicts T-001's defects | **falsified** on C1, **held hard** on C2 (✗101 / M-445) | Only **three of six** orbit labels populated — that is C1's death — but the chi-square reads **445.967** on dof 2 against a critical **5.991**, with the decomposable stratum contributing **327.649** by itself at defect rate exactly **0**. And the row's most reusable output: **zero** non-manifold edges, **zero** non-manifold vertices, **zero** self-intersecting pairs — T-001's entire defect population on these fields is degenerate triangles |
| **P-133** — a filtered exact sign for `Δ`, under 1.5× | **falsified** on C2 **on an integer before any clock is read** (✗102 / M-446) | `naive_ops` **27** against `filter_ops` **65** is **2.407**, already past the registered bar. And the deeper thing: a static filter cannot certify an **exactly zero** `Δ`, and three of eight fields put 84–100% of their surface cells there |
| **P-134** — `|Δ| / (max|fᵢ|)⁴` as a continuous ambiguity magnitude | **falsified** on C2 at 3 of 8 fields against a bar of 4 (✗103 / M-447) | `box_exact` is **arithmetically excluded rather than counted as a miss**: its normalised magnitude has variance **0** over all 1,352 surface cells because a polyhedron's `|Δ| / (max|fᵢ|)⁴` is identically `0`. C1 held exactly — dyadic-scale error **0** on 16 of 16 |
| **P-135** — where the multi-affine identity stops holding | **held**, all three (M-448) | The boundary is now a number: a tricubic residual of **56 named non-zero terms**, and under `smooth_min(k)` the deviation tracks `k` as exactly `k − (7/16)k²` — **9.9956e-4** at `k = 0.001` rising to **3.906e-1** at `k = 0.5`. The `O(k)` seam shell the 2026-08-23 memo measured, joining Group A's scope to `M-38` |
| **P-136** — the hyperdeterminant resolves the interior test | **the registered null came back null** (M-449) | `sign(Δ)` is worth *nothing* on the interior test: the best sign-only rule scores **0.985600** against a base rate of **0.985600** — `delta_information_gain` **0.000000** — and the reason is a group action, not a fitted number: `Δ` is blind to face negation while that same negation moves the separated count from **225** to **4,113** |
| **P-137** — BKK mixed volume 2: at most two critical points | **held**, all three (M-450) | Mixed volume `8 − 6 + 0 = 2`, reproduced independently as an eliminant of degree exactly 2 in `w`. And `SADDLE_COUNT`'s 2 is a **different theorem**: `grad_disc_degree` **6** against `repo_disc_degree` **4**, root sets differing on **31,883** census cells and agreeing on **7** |
| **P-138** — price the tricubic case explosion before anyone proposes it | **held**, all three (M-451) | Tricubic mixed volume **116** against the trilinear's **2**, case space `2^8` → `2^64`, `case_space_tractable` **false**. The unregistered **triquadratic** arm is the only rung with both order and a tabulable table: mixed volume **29** over `2^27` = **134,217,728** |
| **P-139** — is the Kuhn triangulation regular, so Viro patchworking transfers | **held** on C1 and C3, C2 split 4 true / 12 false (M-452) | **Kuhn's six tetrahedra are regular**, exhibited by the lifting `w(S) = −|S|(|S|−1)/2` verified against all 24 inequalities *and* a 70-subset lower-hull enumeration. The registered PW/PV comparison disagrees on **22,841 of 99,932** active cells in **both** directions |
| **P-140** — patchworked fields with prescribed genus, as bench-local fixtures | **held**, all clauses on all **560** rows (M-453) | Six fields with genus **1, 2, 3, 2, 3, 5** prescribed *by construction*, and every one of **7** extractors reproduces every prescribed `χ` — all 56 pairs correct at 49³. Its sharpest finding is not about genus: **`graph_cube_g5`, the highest-genus fixture, is perfect at 7³ and 11³ and meshes literally nothing at 9³** |
| **P-141** — derive the case table by real quantifier elimination, offline | **held** — C1 true on **2,112 of 2,304** queries, the **192** exceptions all the pre-registered outcome (M-454) | z3 5.1.0 re-derived `AMBIGUOUS_FACES` **entry by entry over every real corner assignment**, `cases_matching_table` **1,536 of 1,536**, `cases_disagreeing` **0** — and it **refuted the shipped `Separate` default on 192 of 192 ambiguous faces in 1.185 s**. The 192 timeouts are nested-quantifier *witness* queries at the 2 s cap, `terminated=false`, which is C1's registered second branch. **2,304** hash-stamped `.smt2` certificates |
| **P-142** — the gyroid's `χ = −8N³` under periodic wrap | **held** on C1 and C2, **falsified** on C3 as registered (✗104 / M-455) | `chi_measured` **−8 / −64 / −216** against predicted at `N = 1, 2, 3`, genus `1 + 4N³` = **5 / 33 / 109** on all 16 rows. C3's falsifier's own reason was *too strong*: what the gate needs is a second `ValidateConfig` constructor plus one fold pass, **not** a change to `Extractor::extract_into` |
| **P-143** — Schwarz P `−4` and Schwarz D `−16` as second and third known-χ fields | **falsified** on C1 at **1 of 10** rows, C2 **held as an exact `i128` polynomial identity** (✗105 / M-456) | The one miss: `schwarz_p` at `N = 3` reading **−102** against **−108** with `non_manifold_edges` **6** and `chi_pinch_corrected` **−108** exactly. The mechanism is a group action: **Schwarz P's body-centring operation *negates* `F_P` rather than translating it**, so P's lattice is `simple_cubic` beside space group `Im-3m`, and that apparent contradiction *is* the mechanism |
| **P-144** — can a periodic value-noise terrain admit a χ oracle | **falsified by the three-octave half, and the falsification runs the *interesting* way** (✗106 / M-457) | At **one** octave χ converges on **5 of 5** seeds and `oracle_exists` is **true on 35 of 140** rows, against a registration that named "no oracle" as the likely outcome. The gate must be **per-seed** exactly as registered: converged χ reads **−8** on three seeds, **−12** on one and **−16** on one |
| **P-145** — a digital-topology χ oracle where the crate has none | **held**, all three (M-458) | It reproduces `χ = 2` on sphere and **0** on torus, reproduces P-142's analytic gyroid **exactly** (**−8 / −64 / −216**, `disagreement_cells` **0**), and supplies a first ground truth on the two closed fields the crate declares unknowable — `gyroid` **−24** stable across 33³ and 65³, `noise_cavity` **82** and **−152** — at `cost_ratio` **0.4746–1.0648** of the extraction it grades. Etiene's DOI resolves through three routes to **`10.1109/tvcg.2011.109`** |
| **P-146** — a metric from the trilinear's own Hessian buys 25% fewer triangles at matched error | **falsified** on C1 and C3 (✗107 / M-459) | `c1_winners` **0** of a population of **4** — at matched Hausdorff the metric arm is never cheaper and is **2.298935×** *dearer* on `thin_plate`, the most anisotropic measurable field. `metric_share` **0.332862–8.747515** against a 0.15 bar. And the finding about the roster: the one field where the mechanism demonstrably works, `fbm_terrain` at `flat_axis_aligned_fraction` **1.000000**, is `Unbounded` and therefore ungradeable |
| **P-147** — the exponent does not improve and the constant does | **falsified** on all three (✗108 / M-460) | The exponent moved by **0.352013** on `fbm_terrain` against a 0.1 bar; the fitted constant's correlation to the AM–GM gap is **−0.095238** — *negative*, not merely under 0.7. And C3's `csg_difference` read `exponent_difference` **0.000000** because a permutation-symmetric field annihilates all three degrees of freedom a per-axis grid has |
| **P-148** — metric interpolation across a seam | **held** on C1 and C2, C3 split 84 true / 12 false (M-461) | Component-wise determinant swell reaches **6.84e9** against log-Euclidean's **1.77e-7**; C2 held on **48 of 48** geometries, so three transcendentals cost `M-32` exactly nothing and `hashes_moved` **0**. The twelve C3 failures are `gyroid` under *both* schemes — its seam correspondence is not injective (`seam_vertices_a` **295**, `seam_vertices_b` **294**), which is `M-48` again and not the interpolant |
| **P-149** — the saving is monotone in the principal-curvature ratio | **falsified** where readable at all (✗109 / M-462) | `torus`'s saving is **perfectly anti-monotone**, Spearman **−1.000000** at 65³, and `fbm_terrain`'s `umbilic` bin — the control that was supposed to show *no* saving — costs **−6.04**. **70 of 112 rows read `unmeasurable:arms_identical`** because five of eight fields make P-146's two arms one grid |
| **P-150** — Cao's `L¹` anomaly in Cao's own table | **the registered null came back null and then inverted** (M-463) | The anomaly did **not** reproduce — `anomaly_reproduced` **false** on all 384 rows, `violations_in_l1` **0** of **72** — because `M_L1` wins **all 96 groups in every norm**. `L¹` is the one column where the theory holds rather than the one place it fails |
| **P-151** — goal-oriented adaptivity beats camera-distance LOD by 20% | **falsified** on C1 and C2 at 16 true / 48 false, C3 **falsified** (✗110 / M-464) | It holds on `torus` alone — and there it genuinely is the visibility mechanism, `gain_near` **0.244932** collapsing to `gain_wide` **−0.010015**. But `sphere`'s goal arm is **10.655×** *worse* at matched triangles, and the whole criterion costs **0.371498–3.055830** of extraction with `adjoint_share` **0.144361** against a 10% bar |
| **P-152** — Jones' `β` is the planarity assumption the QEF never checks | **falsified** on C1 and C2, C3 held on **6 rows of 16** (✗111 / M-465) | `β` is not a fraction of an extraction — it is **one to two extractions** (`beta_share` **0.824659–1.693981** against a 0.10 bar) — but the signal it carries that the residual does not is **3 to 176 cells a row** |
| **P-153** — `β` beats curvature as a refinement criterion | **falsified** on C1 and C2 (✗112 / M-466) | The criterion this row was built to promote **loses to the derivative it was built to discredit, on every field** (`beta_beats_curvature` **false on 8 of 8**). And `noise_cavity`'s refinement ladder is **inverted** — error goes *up* under refinement, 0.96 span — which is the sub-cell structure C2 was about, arriving as a measurement of the premise |
| **P-154** — a triangle budget from a convergent sum | **falsified** on C1, C2 held on **21 rows of 24** (✗113 / M-467) | **Unreachable at 4 fields** — `max_fields_any_constant` **4**, so no single `k` could ever have passed — and **`M-13`'s area law, with zero fitted parameters, gets 5 fields where the `β`-sum gets one**. `thin_plate` divergent at all three resolutions |
| **P-155** — predict `M-12`'s constant from Strang–Fix instead of fitting it | **falsified** on C2 at **0 of 8** against a bar of four — **and the bar equalled the entire measurable population** (✗114 / M-468) | C1 HELD: order **2** derived and measured on 8 of 8, both — the hat reproduces constants and linears exactly and fails on quadratics. Two of the four measurable fields are polyhedra whose creases have no second derivative |
| **P-156** — a compact prefilter instead of a truncated recursive one | **falsified** on C1 at **5 of a population of 6** and C3 at **one ulp** (✗115 / M-469) | `csg_difference` at **+6.80%** is the one miss. C3's failure is **not the quasi-interpolant's**: the shipped `standard_trilinear` arm loses seam bit-exactness on the *same six fields with the same worst delta*. `footprint_probe_outside` **0** on all 80 rows |
| **P-157** — raise the order: the lever `✗42` did not pull | **falsified** on C1, C2 and C3 **held** (✗116 / M-470) | The tricubic Lagrange `4×4×4` filter **is** genuinely order-4 on its own zero set (`recon_fitted_exponent` **3.84 / 3.09 / 3.74 / 4.04**), and the mesh's exponent still reads **~1.9** — with an **infinite-order `exact_oracle` arm pinned at the same ~1.9**: *the second order belongs to the piecewise-linear mesh, and no filter can reach it*. `cases_invalidated` **256** against a prediction of **256** |
| **P-158** — sparse grids need a regularity the sharp CSG fields have not got | **the null arrived exactly as registered** (✗117 / M-471) | C1 HELD with the mixed-derivative norm divergent on both CSG fields (`norm_growth_exponent` **5.000000** on `box_exact`, calibrated by a Lipschitz-kink arm reproducing its analytic `6/h^5` to **4.33e-15** relative). C2 FALSIFIED on **8 of 8**: on the three qualifying fields the Smolyak arm is **4.19×**, **4.94×** and **2.26×** *worse* while holding **4.13×** fewer samples, and `break_even_reads_per_eval` **8,793.8** prices the mechanism out of this crate at any accuracy |
| **P-159** — how far from optimal the extractor is | **the phase's most valuable null, now a null with page numbers** (M-472) | Against `W_inf²` — the class the trilinear model is second-order exact for — Marching Cubes is **order-optimal**: `ratio` **1.020157** on `sphere` and **0.991341** on `torus`, with the independent `h` fit reading **1.975** and **1.919** against minimal **2.000000**. Only the constant is in play, and `constant_gap` bounds it at **7.25** and **6.26**. Three things the source does *not* contain are carried in every row's `class_assumptions` rather than glossed |
| **P-160** — does adaptivity beat uniform, given the factor-2 theorem does not apply | **held**, all three (M-473) | The octree-adaptive arm beats uniform on **12 of 12** measured rows, `gain` **1.23–16.97**, with `gain_exceeds_two` true on **10 of 12** — and the factor-2 cap is *measured* not to apply rather than argued away: `class_convex` **false** at `convex_midpoint_min_value` **0.200000000**, `class_symmetric` **false** at `symmetry_signed_volume` **4.179064172** against **−4.179064155**, `operator_linear` **false**, `hypotheses_satisfied` **1 of 4** |
| **P-161** — which approximation class `A^s` each field is in | **held**, all three (M-474) | `s` is estimable for both families on **8 of 8** fields over **2.03 decades**, and the answer is field-dependent: `lod_helps` true on **3** (`box_exact` +0.32, `csg_difference` +0.17, `thin_plate` +0.61, each clearing its own combined half-width) and false on **5**, with `fbm_terrain`'s CI **−0.181 to −0.012**, strictly negative — greedy adaptivity is measurably *worse* than uniform there. The DOF are **field evaluations**, and the adaptive family is charged for its own error estimator |
| **P-162** — BCC vs cubic: `0.2571 dB` is a prediction, not a hope | **falsified** on C1 and C2, C3 **held** (✗118 / M-475) | `fields_improved` **3 of 8** against a bar of five, and the one field that met the prediction met it only because `PROJECTION_STEPS` moved **64 → 1024** — at 64 the projection **aborted the run entirely** (a CSG crease contracts at 0.965 per step) and flipped `csg_difference`'s verdict from **−0.177 dB** to **+0.417 dB**. Stalls fell 240 → 86 with 80 of the 86 at one legitimate local minimum; verdicts identical at 512 and 1024. `case_table_size` **16** against **256** |
| **P-163** — FCC vs BCC is `0.011 dB`, below the harness's scatter | **falsified, and the registered null came back not null** (✗119 / M-476) | `fields_below_scatter` **1 of 8** because `measurement_scatter_db` is **exactly 0.000000** on 6 of 8 fields — a deterministic computation has no scatter, so `gap_over_scatter` reads **inf** and the clause is *arithmetically* false rather than narrowly missed — against `measured_gap_db` reaching **+5.731155 dB** on `noise_cavity`, **514×** the registered prediction |
| **P-164** — the BCC box-spline paper, finally opened | **held** on C1, **falsified** on C2 — **arithmetically unreachable** (✗120 / M-477) | `c1_fields_held` **8 of 8**, both filters order 2, the derivation *executed* to `affine_residual` **4.441e-16**. C2's bar cannot be cleared because `P-162`'s non-control arm was **already** BCC plus the box spline — `hausdorff` reproduces `p-162.csv` to the last digit on all 16 rows, so the bar C2 must exceed *is* the measurement C2 makes |
| **P-165** — greedy meshing against Eppstein's polynomial-time optimum | **held**, all three (M-478) | **Greedy meshing is finished.** Eppstein's optimum reproduces exhaustive search on **33,267** components with `brute_disagreements` **0**, and the shipped mesher's worst distance from it in the whole sweep is `ratio` **1.027132** — **1.120%** over optimal, exactly optimal on 8 of 21 rows — while `cost_ratio` drops to **0.424099**: computing the optimum is *cheaper* than running the mesher on 9 of 24 rows |
| **P-166** — greedy meshing is no matroid; the LOD budget might be submodular | **the registered negative held, the hoped-for guarantee did not** (✗121 / M-479) | `is_matroid` **false** on all 6 rows with augmentation failures **87 / 190 / 2,331** on exhaustively scanned fixtures — the minimal counterexample a `2×2` block, `min_basis` **1** against `max_basis` **4**. C2 FALSIFIED on `violating_chunks` **92 of 221** on `capped_gyroid` and **155 of 186** on `fbm_terrain` against **0 of 152** on `sphere`, with marginal returns that **increase** at the third refinement |
| **P-167** — the Walsh–Hadamard spectrum of the 256-case table | **falsified** on C3 at `eval_ms_ratio` **953.694027**, **held** on C1 and split on C2 (✗122 / M-480) | The two informative bits of the shipped table are **dense** — `spectral_terms` **256 of 256** — while its twelve edge bits are each one degree-2 coefficient. The registration's own guess about C3 was right: **a 256-entry table is already in L1** |
| **P-168** — noise stability against the wrong noise model, on purpose | **the registered null came back registered** (M-481) | C1 HELD on **240 of 240** with `closed_form_max_gap` **0.0** and stability computable a second way to **6e-15**. The genuine artefact at `ulp_perturbation` **1** flips **0** corner signs on all eight fields, so the whole comparison lives on amplified error — and where the model agrees it agrees for a nameable reason: `measured_response_per_corner_flip` **8.000000000** against `total_influence` **8.0** on the parity bit |
| **P-169** — per-corner influence as a cheap independent check on `validate_table()` | **falsified** on C2, **held** on C1 in the strongest reading it can have (✗123 / M-482) | `symmetry_class_count` is **1** — all eight corners are one orbit — so "equal within each octahedral symmetry class" means *all eight equal*, and `influence_spread` is **0.000000000** on every bit of `triangle_counts` with `octahedral_violations` **0** over 12,288 pairs. C1 holding *is* C2 failing: a function whose corner influences are all equal cannot rank corners, and `refinement_priority_correlation` reads **exactly 0.000000** on 200 of 200. The corrupted-table control detects **2 of 4** planted corruptions, all four showing `influence_shift_abs` **0.007812500** = 2/256 |
| **P-170** — Marching Cubes' 1985 ancestor, and how much less it proves | **held**, all three (V-51 / M-483) | Every DOI resolved, **including the registered trap**: `10.1137/0722019` is T. Nanda on the QR algorithm, the article immediately *preceding* Allgower & Schmidt at 322–346 — `trap_gap` **1**. AS85's `is_homeomorphism` is **false**: it constructs a PL manifold along which ‖H(x)‖_∞ < ε under a full-rank hypothesis at the seed. The repository counts are re-taken every run: `descendant_citations_in_repo` **21**, `root_citations_in_repo` **4**, of which **0** survive removing the registration layer that introduced the name |
| **P-171** — how often transversality fails | **held** on C1, **falsified** on C2, C3 split 6 true / 18 false (✗124 / M-484) | `box_exact`'s exact-zero samples make C1 non-vacuous. And the phase-wide fact lands here: the shipped MC emits **zero** non-manifold edges, **zero** non-manifold vertices and **zero** self-intersecting pairs on all eight fields at 17/33/65 — all **227** defect cells are degenerate triangles, on `gyroid` and `noise_cavity` only |
| **P-172** — Conley index dies on plateaus; the census says how big | **the registered obstruction is real, one dimension lower than claimed, and does not stop Conley** (M-485) | `plateau_fraction` **0.500000** on `box_exact` at all three resolutions, `constant_cells` **0** — yet `conley_applicable_fraction` never falls below **0.957031**. C2's ranking came back **reversed**, true on only **3 of 24**, all three `fbm_terrain` at saturated equality — the one vacuous way C2 could hold, which the harness predicted in advance |
| **P-173** — the curl of the Hermite data is what `λ = 0.01` guesses about | **held** on C1 — the curl residual is **free**, worst `curl_share` **0.008061907** — and does not predict `M-60`'s cells (M-486) | Of nine rows with a reachable AUC only `gyroid` at 33³ clears 0.8 at **0.914640664**, and `fbm_terrain` at 25³ reads **0.270443196**, worse than chance. The per-cell `λ` cuts `noise_cavity`'s self-intersections from **388** to **195** at 33³. The harness header was pessimistic by ~2× against its own CSV, and the entry says so |
| **P-174** — varifolds against the normal cycles already benched | **the registered null held**: the normal-cycles incumbent beats discrete varifolds on 28 of 30 rows at matched cost (M-487) | `error_ratio` **1.000821**–**3.358471**; the two exceptions are the `capsule` at 129³ at **0.998923**, a 0.1% win inside a C⁰ seam that dominates both arms. The one recommendation that does move is on a quantity C1 never scored: on the `torus`'s **Gaussian** curvature the varifold overtakes at 47³ and is **1.59×** better at 129³ |
| **P-175** — fix slivers without moving a vertex; `hashes_moved` predicted 0 | **falsified** on C1 and C3, **held** on C2 — **and C2 is the result** (✗125 / M-488) | **15,391 flips** reach the intrinsic Delaunay fixed point on every row, remove **1,570 of 10,678** slivers, and reach **nothing the crate reads** — `vertex_positions_moved` **0**, `hashes_moved` **0**, `extrinsic_geometry_identical` **true**, against a positive control that moves the hash on all 18 rows where a flip happened. The worst-decile minimum angle rises by at most **1.642437°** against a 10° bar |
| **P-176** — caves percolate in 3D and cannot in 2D | **held** on C1 and C3, C2 split 42 / 42 (M-489) | The transition is now a number: on `noise_cavity` the giant component appears at `percolation_isovalue` **0.029340**, and at `iso = 0` one component holds **0.814493** of the air against 65 isolated pockets. Two instruments sharing no data structure agree on the component count, the air-sample count and the full sorted size multiset on **84 of 84** rows. The 2D/3D difference is present on `noise_cavity` (`percolation_isovalue_2d` **none**) and absent on `fbm_terrain`, whose 2D slices percolate too — which the registration named in advance as a real risk, `fbm_terrain` being hash-based lattice noise and not a Gaussian field |

**Five harnesses re-derived a registered threshold against a measurement rather than amending a
registration**, and all five stories are in their entries: `common/lattice.rs`'s `PROJECTION_STEPS`
64 → 1024 (P-162); P-148's `SELF_SWELL_TOLERANCE` `1e-9` → `1e-5` (`H_FLOOR` puts the metric's condition
number at ~`1e9`, so a `log`/`exp` round trip loses `κ·ε ≈ 2e-7` and the old bound was unreachable by
construction); P-153's `LADDER_SPAN_FLOOR` `1.2` → `1.01` and bidirectional (`noise_cavity`'s ladder is
*inverted*); P-156's three population exclusions with reasons; P-164's `LADDER_DENSITY_TOLERANCE`
`0.05` → `0.08` (lattice site counts are quantised, worst rung 6.454%). **No registration was amended.**
`experiment.rs:26-31` forbids it once a run exists, and `M-289`/`P-30`/`P-35` remain the precedent for
re-asking with a corrected instrument.
...
Over the eight fields the roster held at the time, at three resolutions, no chunking and no weld, so there is only one
