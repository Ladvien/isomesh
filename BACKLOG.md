...
| | ID | Ticket | Size | Blocked by |
|---|---|---|---|---|
| ☐ | **E-214** | **The tunnel, on screen — the A-002 series has no visual at all.** `docs/gifs/` holds eight GIFs, newest 2026-08-14, and `docs/screenshots/` holds 49 PNGs of which **none is the tunnel**. When `bevy_isomesh/README.md`'s example table gained a `marching_cubes_tunnel` row at D-011, its Evidence column had to point at the examples catalog because every other row links a screenshot and this one had no image to link. So MC33's interior ambiguity — the headline of 0.0.5 and the largest algorithmic result in the repo — ships invisible. Record a GIF and a still of `marching_cubes_tunnel`: one cell meshed twice, **two discs becoming one cylinder**, the inner hexagon drawn as the closed gold ring. **The caption carries the measured claim, not an adjective** — components **2 → 1**, χ **2 → 0**, boundary edges *unchanged* (M-222), which is crack-freeness visible as a number. Wire it into three places that currently misrepresent the state: `bevy_isomesh/README.md`'s row (replacing the catalog link), the root `README.md` image strip, and the catalog's E-213 row. **Constraint:** capture runs on the desktop X session, so a Bevy window opens on screen for the duration; `ffmpeg` and ImageMagick are present, `gifski` and `wf-recorder` are not. Existing GIFs run 0.7–4.8 MB, which is the size budget. | S | — |
> BLOCKED: **on a capture environment, measured rather than assumed (M-235).** The rig itself works — `ISOMESH_SCREENSHOT` produced a correct 264 KB still of the tunnel through Bevy's GPU readback, HUD and all, so no compositor is needed to *render*. **What does not work is geometry.** This host's `:0` is root-owned and refuses authorization; `:1` is reachable but has **no window manager**, and with none the bare X server fixes the window at 836×1356 regardless — `ISOMESH_WINDOW=1280x720` and `=1600x900` both returned exactly 836×1356, portrait, and the override reported no error. That is the one thing `size_window`'s own doc comment says it exists to guarantee, so a capture taken here is neither landscape nor reproducible. **Unblock is one package** — `xorg-server-xvfb`, then run the capture against an `Xvfb :99 -screen 0 1600x900x24` display, which also makes the recipe CI-runnable and never touches the desktop. **Not installed unilaterally:** a system package is the user's call. The alternative is running the capture on the real desktop session, where a window manager is present.
| ☐ | **E-215** | **Write the GIF recipe down — the mechanism exists in code and the method does not.** ~~Grepping for `ffmpeg` returns nothing, so every GIF was made by a procedure that survives only in whoever ran it.~~ **Half wrong, corrected 2026-08-15 before starting.** `examples/common/mod.rs` already carries a complete capture rig — `ISOMESH_CAPTURE=<dir>` dumps `frame_NNNN.png` through Bevy's own screenshot path with `ISOMESH_CAPTURE_FRAMES`, `_EVERY` and `_SETTLE` knobs, plus `ISOMESH_SCREENSHOT` for a single still and `ISOMESH_SPIN` to orbit. What is undocumented is the **assembly**: nothing anywhere says how a frame directory becomes one of the eight committed GIFs. That is the same class of gap `FINDINGS.md` Part 5 exists to close, applied to an artefact rather than a measurement. A `scripts/record_gif.sh` taking example, duration and output path — x11grab capture, two-pass `palettegen`/`paletteuse` because a single-pass GIF of a shaded 3D scene bands badly, and a size assertion against the 0.7–4.8 MB budget the existing eight sit in. Plus one line in the examples catalog saying which script made the images, so the ninth GIF is a command rather than archaeology. **Acceptance:** re-recording an *existing* GIF with the script produces a file within the size budget and visibly comparable to the committed one — that comparison is what proves the recipe is the recipe, rather than a plausible script nobody checked. | S | E-214 |
