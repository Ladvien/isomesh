...
**89 tickets archived, 6 open.** Completed rows move to `BACKLOG_ARCHIVE.md` with their amendments
...
| ☐ | **GPU-011b** | **The general mechanism: a field a consumer supplies, evaluated on the GPU.** GPU-011a proved the win — 15.5× at 129³, 0.54 ms end to end (M-155) — with four fields hard-coded in WGSL. That is not shippable: a consumer's own SDF cannot be one of four. **Two designs, and the choice is the ticket.** *(a) Interpret an edit log.* This crate's world already **is** a base field plus an ordered op list — `BrushStack`, and `paint::Edit` after E-208 — so upload the ops and interpret them in the shader. Generalises to any composition of the primitive set, is testable exactly against `brush::apply`, and is what the game audience actually has. Bounded by the primitives the shader knows. *(b) Compose the consumer's WGSL* through GPU-002's `#include`. Unbounded generality, zero interpretation cost, and the field is then written **twice** — once in Rust, once in WGSL — which is the drift risk rule 5 exists for. **Mitigable and worth saying so:** the drift is *testable*, by sampling both and comparing, which is exactly what GPU-011a's deviation harness already does. **What M-154 contributes to the choice:** the arithmetic drift is a property of the *expression*, not of the GPU — a field of `abs`/`max`/`min` crosses within 1 ULP, one of sums-of-products does not — so an interpreter built from exact operations would drift less than compiled WGSL doing the same maths. **Acceptance:** one design, argued in writing against the other; a consumer field evaluated on the GPU; the deviation table extended to it; and the mesh asserted unchanged, as M-154 does for the built-ins. | L | GPU-011a |
