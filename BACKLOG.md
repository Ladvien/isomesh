...
**90 tickets archived, 5 open.** Completed rows move to `BACKLOG_ARCHIVE.md` with their amendments
...
| ☐ | **GPU-008b** | **`E-303 gpu_mesh_shader` — the Bevy example that actually draws with it.** Split from GPU-008 once the shader and pipeline landed (GPU-008a): what remains is Bevy-side plumbing rather than shader work, and it is the half that cannot be tested headlessly. Needs a render-graph node driving `MeshShaderRenderer` against a view target, since Bevy's `Material`/`Mesh3d` path cannot express a mesh pipeline. **Acceptance:** the extracted surface is drawn from the compute output's own buffers with no vertex or index buffer, and the HUD says which pipeline ran — on an adapter without the feature it says so and shows the compute path, with the choice visible rather than silent. **Its value went up sharply and the note that said otherwise is stale.** With the field evaluated on the GPU (GPU-011a) the whole extraction is **0.54 ms at 129³ and leaves the geometry in GPU memory**; a consumer who then needs it on the CPU pays a `~0.63 ms` geometry read-back on top, so drawing with a mesh shader instead removes **about half the remaining path** rather than the 8.1% an earlier note claimed. The pipeline is no longer a precondition for someone else's win — it is how the 0.54 ms path gets used at all. **The Bevy 0.19 route is known and is not the old `Node` trait** — `RenderGraph` is a `ScheduleLabel` and `RenderContext` is a `SystemParam` giving `command_encoder()`, so this is a system in the `RenderGraphSystems::Render` set, not a graph-node impl. | L | GPU-008a |
