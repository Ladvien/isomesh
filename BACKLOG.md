...
**88 tickets archived, 6 open.** Completed rows move to `BACKLOG_ARCHIVE.md` with their amendments
...
| | ID | Ticket | Size | Blocked by |
|---|---|---|---|---|
| ☐ | **GPU-010b** | **An indirect mesh draw, so the last four bytes stop coming home.** GPU-010a moved the scan onto the GPU; what remains is that the CPU still reads the grand total to size the geometry buffer. Write the total and the mesh-workgroup count into an indirect buffer and draw with `draw_mesh_tasks_indirect` (`wgpu-29.0.4`, `api/render_pass.rs:303`), and the extraction reads back **nothing**. **The sizing question is the real content of this ticket, not the draw call:** without the total on the CPU the geometry buffer has to be allocated worst-case, or grown and the emit pass re-dispatched, and that is a decision with a memory bill attached — at 129³ the worst case is 12 triangles per cell against a measured 38,456 total, which is four orders of magnitude of slack. **Acceptance:** a 129³ extraction with zero read-backs, measured against `gpu_vs_cpu.csv`, and the allocation strategy written down with its cost. | L | GPU-008b, GPU-010a |
| ☐ | **GPU-009** | **Mesh-shader kernels cannot be executed under test, and that is a coverage hole rather than a missing convenience.** `isomesh-gpu::headless::Gpu` cannot open a device with `EXPERIMENTAL_MESH_SHADER`, because the token's only constructor is a `const unsafe fn` and this workspace sets `unsafe_code = "forbid"` (M-146). The consequence is precise: a mesh-shader kernel can only ever be *run* inside a Bevy app, on a machine with a capable adapter — so there is no headless execution test for one, and nothing about its output can be asserted in CI. **GPU-003's naga sweep is unaffected**: it parses and validates without a device, so shader *validity* stays covered; what is uncovered is *behaviour*. **Options, and the choice is a policy one:** relax `unsafe_code` for `headless` alone, deliberately and in writing, with the `unsafe` confined to one documented call; or accept the hole and say so on `headless::Gpu`, so nobody assumes the harness covers what it does not. Do **not** resolve it by having the headless path silently fall back to compute — that is the two-path failure the one-path rule exists to stop. | S | GPU-008a |
| ☐ | **GPU-011a** | **Evaluate the field on the GPU — prove the win before designing the general mechanism.** After GPU-010a the upload is **86% of the extraction path** (7.04 ms of 8.15 at 129³ after GPU-012), and nearly all of what remains is `field.sample()` running on the CPU — the byte-shuffling half was removed at GPU-012 and the `write_buffer` memcpy is irreducible (M-152, M-153). Evaluating the field in a compute pass removes the upload **entirely** — the samples are written where they are needed and never cross the bus. `FieldBuffer` was built for this: it already carries `STORAGE \| COPY_DST \| COPY_SRC` and its docs already say *"usable as a compute output — a shader that evaluates the field on the GPU writes here"* (GPU-001). **Do the cheapest version first**: one hard-coded analytic field in WGSL, a `sample_field` pass that fills a `FieldBuffer`, and a measurement. **Pre-registered prediction, before any of it is written:** upload falls below **0.5 ms**, the whole 129³ path lands under **2 ms**, i.e. **≥4×** on top of GPU-012 and ≥7.5× on the pre-scan baseline. **Acceptance:** that number, confirmed or falsified, plus the deviation table below. | M | GPU-010a |
> **It costs the sample-identity that makes GPU-005's comparison mean anything, and that has to be measured rather than assumed.** Today the CPU and GPU read *the same `f32`s* — the CPU produced them — which is why M-142 could report vertex positions agreeing to ≤1 ULP and attribute the residue to fused multiply-add. Evaluate the field independently on each side and the agreement becomes a function of each platform's arithmetic. `sqrt` is correctly rounded in IEEE and may well still match; **WGSL's `sin`/`cos` have implementation-defined accuracy bounds and `libm`'s do not agree with them**, so `gyroid` is where this will show. This does not touch the CPU path's determinism guarantee — `libm` is still bit-reproducible across machines (M-31) — but **the GPU path acquires no such guarantee**, and saying so is part of the ticket. Deliverable: a per-field deviation table, not a bit-identity claim.
| ☐ | **GPU-008b** | **`E-303 gpu_mesh_shader` — the Bevy example that actually draws with it.** Split from GPU-008 once the shader and pipeline landed (GPU-008a): what remains is Bevy-side plumbing rather than shader work, and it is the half that cannot be tested headlessly. Needs a render-graph node driving `MeshShaderRenderer` against a view target, since Bevy's `Material`/`Mesh3d` path cannot express a mesh pipeline. **Acceptance:** the extracted surface is drawn from the compute output's own buffers with no vertex or index buffer, and the HUD says which pipeline ran — on an adapter without the feature it says so and shows the compute path, with the choice visible rather than silent. **Expect a small win and say so on screen: after GPU-010a it removes 0.78 ms of a 9.65 ms path at 129³, 8.1%** (M-150), and the dominant cost is now the upload at 87%; the pipeline is a precondition for GPU-010 rather than the win by itself. **The Bevy 0.19 route is known and is not the old `Node` trait** — `RenderGraph` is a `ScheduleLabel` and `RenderContext` is a `SystemParam` giving `command_encoder()`, so this is a system in the `RenderGraphSystems::Render` set, not a graph-node impl. | L | GPU-008a |
