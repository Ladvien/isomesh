...
| | ID | Ticket | Size | Blocked by |
|---|---|---|---|---|
| ☐ | **A-002** | **MC33 / asymptotic decider.** Face saddle `S = (f00·f11 − f10·f01) / (f00 + f11 − f10 − f01)`, guard the denominator. Interior ambiguity via the trilinear body saddle. **Re-scoped — the original acceptance was unsatisfiable, see ✗11.** A-001 cannot hole: `validate_table()` checks all 256 cases and `mc/tests.rs:30` asserts `face_disagreements == 0`, because a face's segments are a function of that face's own four corner signs. So no field holes under A-001 and "closed under A-002" can never be demonstrated. What MC33 actually buys is **topological agreement with the trilinear interpolant**, not crack-fixing. **Acceptance:** a cell configuration with an ambiguous face on which A-001 and A-002 report **different χ**, with A-002 matching the trilinear connectivity — a test where both agree isn't testing this. Spend the `L` knowing the game-facing value is consistency, not hole-closing. | L | A-001 |
...
| ☐ | **A-007** | **Dual Contouring — the vertex solve.** One unconditional path: `M = Σnᵢnᵢᵀ`, `g = Σdᵢnᵢ`, `x = c + adj(M+λI)·g / det(M+λI)`, λ = 0.01. **Merged with the former A-008 — there is no closed-form/general split and no triple-product branch.** The audit doc gives this form as "branch-free, handles all degeneracies… no data-dependent branch", and its own diagnosis is that a data-dependent branch is what makes DC pop: 454 rank-branch disagreements under rotation, median displacement 2.13 cells, max 9.10. Its measured equivariance residual has the Tikhonov form beating the three-plane Cramer form on both tails (p99 1.81e-04 vs 7.23e-04; max 6.4e-04 vs 3.6e-01), so dropping the fast path costs nothing. **Magnitude-sort the 3-term dot products** — 4328/9600 equivariance failures unsorted, 0/9600 sorted; without it the guarantee does not hold in f32. **λ = 0.01 is not arbitrary** — it reproduces DC's σ = 0.1 truncation smoothly, and the corpus circulates three values (0.01, 0.1, σ=0.1). **Document that `M = AᵀA` squares the condition number** — that's why QR/Givens exists in the literature, and why f64 matters here. **glam does NOT land here — see ✗16:** it has no scalar abstraction (`Mat3` is f32, `DMat3` is f64, no generic), so it cannot serve a crate generic over `Real`. The 3×3 is ~40 lines over `[R; 3]` and the crate stays at one dependency. | L | A-006 |
> IN PROGRESS: the vertex rule has landed in `dc/solve.rs` with 11 tests, including bit-exact equivariance across all 24 lattice rotations. **What remains is the extractor**: dual topology is identical to Surface Nets (V-19), so `sn.rs`'s sample/smooth/emit stages should be factored into a shared engine parameterised by the vertex rule rather than duplicated — `place_vertices` is the only stage that differs. Then the T-001 sweep, T-004 determinism, T-005b coverage and a benchmark row.
| ☐ | **A-009** | **The cell clamp + measurement.** Clamp the solved vertex to (1−ε) inside its own cell, ε≈1e-4, behind a flag. **Measure self-intersections per 1,000 triangles with and without, on all seven fields, and put both numbers in the commit message.** Cheapest high-value experiment in the project — it decides whether guaranteed intersection-free extraction is free. | M | A-007, T-002 |
| ☐ | **A-010** | **Manifold DC.** Vertex splitting so each cell can emit >1 vertex where topology demands it. **Acceptance:** `non_manifold_edges == 0` on `gyroid` and `csg_difference`, where plain DC will not manage it. | L | A-007 |
