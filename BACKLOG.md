...
**100 tickets archived, 10 open.** Completed rows move to `BACKLOG_ARCHIVE.md` with their amendments
...
> NOT BLOCKED — **P-7 answered it, which is what P-7 was registered for (M-182).** The note here said the remedy was a design choice that *"puts at risk the exact case A-014e's per-triangle vote was written to protect"*, and that the missing piece was a decision about which guarantee to keep. It was not a decision, it was a measurement, and P-7 had already named its own falsifiers: welded `thin_plate` is **one component, 0 boundary edges, χ = 2, 0 non-manifold edges and 0 non-manifold vertices at 17³, 25³ and 33³**, and the component count stays 1 at every epsilon from `h·10⁻⁹` to `h·10⁻²`. A closed genus-0 surface joined at a rim is one component *by topology rather than by tolerance*, so component-wise propagation is safe on the field the concern was raised about. **Two things sharpen the ticket.** The census `17³ → 0, 25³ → 8, 33³ → 6` shows `thin_plate` itself is not coherently wound at the two resolutions the suite never tested (T-010), so the local vote is failing on its own protected case — an argument *for* propagation. And the placement is settled: it is a **post-weld pass** the caller composes, as `weld` is composed after `extract` today, because connectivity is what propagation needs and the per-tetrahedron soup has none until it is welded.
