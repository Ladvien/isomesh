...
**173 tickets archived, 7 open.** Completed rows move to `BACKLOG_ARCHIVE.md` with their amendments
...
| ☐ | **A-023** | **`emit_quads` is 82% of the dual's cycles and 45% of its instructions, at IPC 0.72. Make it stop.** R-007 (M-284) decomposed `DualMesher` by iteration count without touching it: `sample` runs at IPC 6.49, `place_vertices` at **3.83** — Marching Cubes' own 4.04 — and `emit_quads` at **0.72**, which is what drags the whole mesher to 1.20 and is the entire answer to O-11. **What is wrong with it, from the source rather than guessed:** it walks **every grid edge on all three axes**, `O(n³)`, where the surface is `O(n²)`; it computes two `shape.linearize` calls and loads both endpoint samples **before** the `inside0 == inside1` test that would let it skip; and two of its three passes have their innermost loop on `v = (axis + 2) % 3`, giving strides of `nx` and `nx·ny` where the third runs along `x`. **Three candidate remedies, in increasing order of disruption, and the ticket is to measure them rather than to assume one:** (a) fuse the edge sweep into `place_vertices`' cell loop, which already visits every cell and already has the corner values in registers; (b) keep the sweep and reorder the loops so the innermost index is always the fastest-varying axis; (c) drive it from the crossed-edge list `place_vertices` could record as it goes, making the stage `O(surface)`. **Acceptance:** `cargo bench --bench experiment_p15` re-run, with `emit_quads`' share and its IPC reported before and after; `cargo bench --bench family` re-run and M-282's table updated; **T-007's golden hashes unchanged**, because this is an optimisation and any change to the mesh is a bug in it. **Note what is *not* claimed:** M-279 measured that the dual's cache misses are largely hidden — a 2.4× swing in misses moved cycles by 0.4% — so (b) alone may buy nothing, and that is a result worth having either way. | M | R-007 |
...
| | ID | Ticket | Size | Blocked by |
|---|---|---|---|---|
| ☐ | **R-008** | **The crease is bridged by triangles that face somewhere else — is that inherent?** R-006 (M-283) measured **6,959 vertices under Marching Cubes and 4,868 under Dual Contouring** whose area-weighted normal is more than 90° from the field gradient, worst **128.0°**, on an exact convex wedge. That should be impossible: an area-weighted normal is a convex combination of incident face normals, so it lies inside the cone the two planes span, which is at most 75° wide. The obvious escape was measured and closed — the median `\|f(v)\|/h` at those vertices is **0.0000**, so they are on the isosurface, not off it. So the incident faces are not confined to the two planes: something in the crease cells faces backwards. **H, to be pre-registered as P-16 before anything is measured:** every past-90° vertex lies in a cell that contains **both** faces of the wedge, and the offending triangles are the ones spanning them — which makes this M-15's *"any feature thinner than one cell forces two sheets through it"* on a sharp field rather than a new mechanism, and inherent to one vertex per crossed edge. **Harness:** for each such vertex, classify its incident faces by which plane they lie on (within a tolerance) and record how many lie on neither; check whether the cell's eight corners straddle both planes. **Falsified by:** past-90° vertices in cells that touch only one face — which would make it a winding or ordering defect with a fix, and a much better outcome. **Records:** offending faces per vertex, cell straddle yes/no, to `docs/experiments/p-16.csv`. **FINDINGS:** `M-`, and `✗` against M-283's reading if the cells turn out to be single-sided. | M | R-006 |
