...
**96 tickets archived, 13 open.** Completed rows move to `BACKLOG_ARCHIVE.md` with their amendments
...
| ☐ | **A-014h** | **Subgrid MT — a crossing at an edge endpoint is named by the grid point it lies on, not by the edge it was found along.** M-169's remedy, which that finding names but does not do, and which the 2026-08-14 review flagged as having no owner. Identity-based sharing (A-014g) is complete exactly when no root lands on a grid sample point: a root at parameter 0 or 1 sits on a grid point that up to 24 tetrahedron edges meet at, so it has one position and a different `(edge, index)` name on each — one point wearing many names, which no correct sharing under this key can merge. The correlation is exact, not approximate: `box_exact`, where *every* raw vertex is on a lattice point, loses 73% of them to the positional weld (1262 → 338), while `torus` and `fbm_terrain` (0 on lattice) weld nothing at all. This is a second keying rule, and M-169 is explicit that it belongs in its own commit. **Acceptance:** `how_complete_the_shared_table_is_against_a_positional_weld` reports the on-lattice residue at 0 on all seven fields; subgrid golden hashes re-blessed deliberately (raw vertex counts drop), with the triangle set unchanged — A-014g's own acceptance shape. | M | A-014g |
