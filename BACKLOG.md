...
**129 tickets archived, 3 open.** Completed rows move to `BACKLOG_ARCHIVE.md` with their amendments
...
| ☐ | **A-020b** | **The disk triangulation for a six-saddle cell that is not a tunnel, which Grosso does not give.** A-020 established the classification: a cell with an inner hexagon and a contour past Corollary 6's bound of six is **not** a tunnel — its inside region is two blobs rather than one cylinder (M-229) — and `Contours::topology` now says `Topology::SeparateDisks` there. `extract` returns `Error::UnresolvedSixSaddle` rather than meshing it, because **none of the three published rules applies**. §5.1 triangulates a tunnel against the inner hexagon and this is not a tunnel; §5.2 triangulates the twelve-vertex contour and this has two contours; §5.3 fans a disk from an interior vertex it selects from *face pairs whose quadratic has a single solution*, and a six-saddle cell has none — all three have two, which is what `M-216` pinned as the unreachable five-and-six-line arm. So the construction stops, and the work is to derive what it stops short of rather than to transcribe it. **What is known and should be used:** the inner hexagon exists and lies on the level set, its six vertices are saddle points, and §5.2 already merges them into three inner vertices for the twelve-vertex case — so the hexagon is the obvious source for the added vertices, but *which* of its points serve which contour is exactly what is unstated. Rule 5 forbids picking one; derive it, or take the rule-5 stop and say so. **Grade it the way A-002g and A-002h were graded**: any added vertex must lie on the level set to the tolerance M-216 measured, and the resulting patch must be manifold inside the cell. **Reachable but not by sampling** — 400,000 uniform random cells produce zero, and the fixture in `a_nine_and_three_cell_is_refused_before_triangulation` was found by search over rounded corner values, which is the fixture trap of M-228 in its seventh appearance. | M | A-020 |
