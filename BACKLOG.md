...
**183 tickets archived, 17 open.** Completed rows move to `BACKLOG_ARCHIVE.md` with their amendments
...
> BLOCKED: **on the decomposer choice, which is a design decision and not a measurement — and the ticket's own pick is falsified (✗21, 2026-08-16).** All three claims this ticket cites for CPD were checked against `10.48550/arXiv.2602.07369` and **all three are true**. The architecture on top of them is not. CPD produces **no interior partition**: Algorithm 1 merges the *face adjacency graph* bottom-up, §3.4 states *"Each primitive covers all vertices of each face it subsumes, allowing vertices to be covered by multiple primitives and the union of all primitives covers the mesh's **surface**"*, and §3.3's own *"Overlapping Primitives"* paragraph concedes the cost function *"does not account for double-counted volume of intersecting primitives"*. So there are no cells to cut, *"a fragment is a set of cells on one side"* has no referent, and **`volume conserved to 1e-3` is unreachable by construction** — an enclosure guarantee makes the proxy a strict superset of the solid. The tension is structural: not needing an interior is exactly what makes CPD tolerant of the dirty input ✗20 credits it with. **Three routes, and picking one is the user's call, not this ticket's.** (a) Decompose with a partitioning method — CoACD or V-HACD, per CPD's own §2 (*"remesh or voxelize the input to make it manifold, then partition the manifold mesh top-down along cutting planes"*) — and pay the repair pass ✗20 says is a quality lever rather than a gate. (b) Follow Müller 2013's VACD directly, which is what the architecture was derived from and was never audited here. (c) Re-scope: keep CPD for AG-007-shaped colliders, where every verified claim still holds, and drop the cutting-substrate role. **Nothing downstream should be built until this is settled** — B-013 renders these cells and is blocked behind it.
| ☐ | **T-023** | **Publish the gate-selection rule. RE-SCOPED 2026-08-16 — the split this asked for already exists, and the defect is visibility (✗22).** ~~Split `MeshReport` into a solid report and a surface report.~~ The premise was that a closed-solid test is applied here to meshes that are not solids. **It is not.** `is_manifold` is already documented *"the gate for open fields"* and ignores `boundary_edges`; `is_closed` is *"the gate for closed fields"*; `violations()` already excludes the recorded-not-asserted metrics; and `SurfaceGate` is a **three**-case enum — the third, for a grid that may not resolve the field, is one no two-type split can express. Two report types would be a second path to a question already answered once, and would re-baseline every golden hash, since `MeshReport`'s `Display` is hashed. **The real defect, and it is worse than "private":** `mod property;` is `#[cfg(test)]` (`lib.rs:129`), so `SurfaceGate` **does not exist in a release build at all** — the gate rule is test-only scaffolding, and publishing it is a promotion into the shipped API rather than a one-word visibility change. `MeshReport` and `ReferenceField::closed_in_domain` are public — so a consumer gets three predicates and **no reachable statement of which to call**, and re-derives the rule wrongly. That is the downstream "2/12 manifold" reading, and **this repo's own `bevy_isomesh/examples/manifold_check.rs:379-381` makes the same mistake**, hand-rolling `if is_closed() … else if is_manifold()` twenty lines from the typed rule that exists. **Acceptance:** the gate is reachable from outside the crate and selected from field/algorithm metadata rather than caller intuition; `manifold_check.rs` uses it instead of its if/else; and the docs state which artefact each predicate belongs to — the one clause of the original acceptance that survives. **Open design question, flagged not decided:** the exact public shape — export `SurfaceGate` as-is, add `MeshReport::satisfies(gate)`, or both. | M | — |
