...
| ☐ | **T-026** | **Mesh-quality metrics: aspect ratio and sliver fraction, recorded not gated.** **Was `H-003`; see the note under R-026.** The validity harness reports topology and accuracy and says nothing about triangle *shape* beyond `degenerate_triangles`, which is a near-zero-area count relative to `cell_size` and is not comparable with anything published. **The metrics the literature actually tabulates are grounded in this repo's own corpus**, in `docs/research/2026-08-10-meshing-algorithm-catalog-v2.md`: **AR > 4** as a percentage of triangles, and **sliver percentage** — QUADify measures DMTet at **16.89%** slivers and TetWeave measures DMTet(128³) at **12.35%**; TetWeave's own AR>4 moves **2.251% → 17.091%** across an ablation. **Corrected before anything was built on it — V-38.** Those figures come from a **differentiable-rendering optimisation loop** on learned SDFs, and the `11.46%` is the `MC + Reg.` row rather than plain Marching Cubes, so they are **not a baseline this crate can be measured against**; matching the definition makes the column *readable*, not the values *comparable*. The honest comparison is **within this crate**, seven extractors on one fixture, which is the shape of every other table in the head-to-head. **Recorded, never gated** — the same standing `degenerate_triangles` already has, and for the same reason: Marching Cubes emits slivers whenever a grid corner sits near zero, and that is the algorithm rather than a defect. **The definition is pinned to a tool, not a formula, and that is the first task.** FlexiCubes' Fig. 15 names **PyVista**, which wraps VTK's **Verdict** library — so the target is Verdict's triangle aspect ratio, not any of the three plausible textbook forms (longest edge ÷ inradius, circumradius ÷ inradius, longest edge ÷ shortest altitude). **Confirm it from Verdict before writing code**; rule 5, and a differently computed ratio is an incomparable column. **Acceptance:** both metrics on `MeshReport`, pinned per extractor × field in the golden fixture, and the definition of each stated in its doc comment with the source it matches. **FINDINGS:** `M-`. | M | — |
