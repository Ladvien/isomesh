...
**51 tickets archived, 30 open.** Completed rows move to `BACKLOG_ARCHIVE.md` with their amendments
...
> **BLOCKED: the corpus is unreachable.** `10.48550/arXiv.2606.00454` §3.1–3.3 is where the boundary-curve
> reconstruction and the Steiner-point placement live, and home-still's token has expired. Hard rule 5
> forbids inventing a construction, and this is the one ticket in the queue that is *entirely*
> construction — A-014a deliberately took the part that could be derived, and what is left cannot be.
>
> **To unblock:** re-authorise home-still, then read §3.1 (curves on the tet boundary, normal and
> non-normal parts) and §3.2–3.3 (filling a curve, Steiner points, the intersection-free argument).
> A-014a already supplies the encoding, the validity conditions, and the target to check against —
> M-67's 95.6% is the share of configurations this path has to handle that a lookup cannot.
