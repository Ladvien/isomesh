...
**120 tickets archived, 6 open.** Completed rows move to `BACKLOG_ARCHIVE.md` with their amendments
...
|---|---|---|---|---|
| ☐ | **A-002e** | **The eighth reference field — one with tunnels in it.** M-40 measured that five of the seven reference fields have **zero** ambiguous faces, and only `gyroid` (0.515%) and `fbm_terrain` (1.532%) reach one at all. Interior ambiguity is rarer still: A-002d measures **1,140 of 200,000 uniformly random cells (0.57%)** with six body saddles, and a signed-distance field is far from uniformly random. So the T-001 field gates and the golden fixture may never exercise a tunnel, and a per-cell proof would be all this series ever gets. **Search for the field, do not design it** — sweep candidate parameterisations with A-002d's `BodySaddles` and take one that reaches six saddles at a committed resolution; `gyroid` is the natural starting point. Needs `ReferenceField` metadata and a real analytic `gradient()` (`fields/tests.rs:61` checks it against a central difference). **Acceptance:** `the_tunnel_field_actually_contains_tunnels` asserts a non-zero count — M-44's rule, a measurement that comes back zero has to prove it could have come back non-zero, and this is the fixture trap's sixth outing. **Costs, stated up front:** golden goes 7×8×3 = 168 → 8×8×3 = **192 rows** and `every_combination_is_covered` hard-codes the product; `shootout.rs` and `stage_breakdown.rs` sweep via `for_each_reference_field!`, so their CSVs gain rows and `2026-08-13-measured-comparison.md` must be updated with them. Its own commit deliberately: 24 added rows and zero changed hashes is a readable diff, folded into the algorithm it is not. **If no reachable field produces a tunnel, that is the finding** — it extends M-40 from faces to cells — and this degrades to the per-cell sweeps rather than being forced. | M | A-002d |
| ☐ | **A-002f** | **Contours — the cell's cut edges as closed rings.** Grosso's Algorithm 1 step 2. Walk the cut edges into up to four closed contours, pairing them on ambiguous faces with the **existing** `ambiguity::joined_mask`. No triangles yet. **Crack-freeness is inherited here rather than argued:** the face pairing is untouched, so an interior classification cannot move a face segment, and ✗11's face-locality property plus `validate_decider_table()`'s 16,384 combinations keep covering it. **This is also where the tunnel is finally distinguished from the twelve-vertex contour**, and it is why A-002d does not try: with six body saddles the cell is one or the other, and the discriminator is the **number of contours**, not the asymptote-side predicate Proposition 1 states in prose and never pins down (V-31). **Acceptance:** contour vertex counts checked against Corollaries 2–7 exhaustively over 256 cases × 64 masks, reported as counters in the shape of `validate_decider_table` rather than gated by a blanket rule. | M | A-002d |
