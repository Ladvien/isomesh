...
**99 tickets archived, 11 open.** Completed rows move to `BACKLOG_ARCHIVE.md` with their amendments
...
> BLOCKED: **on a geometry decision, and the measurement that forces it is done.** An exact identity rule — the ticket's original shape — reaches 1028 of `box_exact`'s 1262 raw vertices where the acceptance asks for 338 (M-180), so it cannot be finished as written. Closing the remaining 690 means the extractor emits the grid point's canonical position for an endpoint root instead of the lerp's answer, moving those vertices by a few ulps. That is a change to what the extractor outputs, not to how it names things, and this repo does not make that call silently. **A partial implementation was written and reverted**: bitwise detection against `corners[a]`/`corners[b]` took `box_exact` 1262 → 1100 and `csg_difference` 1280 → 1148, met no part of the acceptance, and moved the raw census (`box_exact` gained 216 inconsistently-oriented edges that a coarser sharing had simply hidden) — half-finishing with a moved census is what rule 4 forbids.
