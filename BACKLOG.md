...
**197 tickets archived, 16 open.** Completed rows move to `BACKLOG_ARCHIVE.md` with their amendments
...
> **PRIOR ART READ 2026-08-16, and the ticket's own flagged risk is resolved (V-43).** Both papers this row calls **unobtainable** are in the corpus and were **indexed on 2026-08-15**, the day before this phase was written — plus a third copy, the 2013 journal version of the first. V-29's incident in a second place, and the same rule: check the corpus before recording a source as unavailable.
>
> **They narrow the novelty claim and do not close it.** Both prove `O(log Δ)` per update, the first is **time-optimal with an `Ω(log Δ)` lower bound** and size-optimal, and both name **self-adjusting computation** — the second says outright that its update algorithm *"is inspired by recent advances on self-adjusting computation"*. So *"the first incremental meshing bound"* is **not available** and neither is priority over change propagation as a technique. What survives is narrower and real: their input is a **point set whose points move**, their output a **quality mesh of the domain**; this ticket's input is a **scalar field on a fixed lattice whose values change**, and its output an **isosurface**, whose combinatorics come from sign changes rather than Voronoi cells.
>
> **Their lower bound does not bind this ticket, and the reason is mechanical rather than hopeful.** `Ω(log Δ)` is proved by exhibiting updates that *"require `Ω(log Δ)` **Steiner points** to be inserted"*; a fixed lattice inserts none, so the mechanism is absent and `O(|edit|)` with no logarithm is consistent with it. **M-311 is already one measured instance** — connectivity repair on a lattice at `O(|edit|)`, no log factor. Note `log Δ` on an `n³` lattice is `log n`, about 11 at 128³, so this would have mattered had it applied.
>
> **What may be claimed after this:** the first incremental bound for **isosurface extraction from a scalar field**. Not more.
