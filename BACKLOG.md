...
**196 tickets archived, 17 open.** Completed rows move to `BACKLOG_ARCHIVE.md` with their amendments
...
> **PROGRESS 2026-08-16 — the reframing landed, P-24 died on arithmetic, and the scalar is now bounded (V-42, M-312).** `Interior::test()` was never a table lookup: it is a **sign test on a value**, so a threshold is a **one-parameter family containing the published decider** rather than a replacement for it. `SweptFaces::margin()` now returns that value and `test()` is literally `margin() > 0` — structural, so the two cannot drift, and R-023's *"reproduces MC33 where MC33 is agreed"* clause is true **by construction** rather than by census. **The margin is bounded by half the field's scale**: 0 of 25,085 ambiguous cells exceed it, max 0.436 against 0.5, and the pole is present on **half** of those sweeps without breaking it — ambiguity itself supplies the bound, since one diagonal strictly negative and the other not makes the denominator's terms add rather than cancel. **Near-degenerate margins are a quantisation phenomenon** — under `1e-6`: 0, 1, 27, 371 as the quantum coarsens — which is where a threshold would act and is the same pattern M-220, M-232 and M-310 found.
>
> **Two things this did NOT establish, and the ticket invites both overclaims.** It is a **decision margin**, not a persistence: `|margin|` pairs a value with the *isovalue*, where a persistence pairs two critical points (V-42). And it does **not** retire A-002b, A-002i or A-020b — those decide *which topology to emit*, this reports *how confidently the cell was classified*. **What remains is the whole second half**: what a non-zero `ε` does to emitted topology, and whether the differences fall only on cells where published algorithms disagree. That is now registerable against a quantity known to exist and known to be bounded, which it was not when this ticket was written.
