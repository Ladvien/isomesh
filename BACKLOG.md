...
**107 tickets archived, 3 open.** Completed rows move to `BACKLOG_ARCHIVE.md` with their amendments
...
> BLOCKED: **on the acceptance criterion, not on the source — and the source is no longer the problem (V-28).** The inset was implemented three ways and measured (M-194). It works, and it does not fix the row the ticket names. **`csg_difference`'s 3 non-manifold edges are not caused by duplication**, and this repository already measured that: `the_defects_traced_back_to_the_tetrahedra_that_made_them` reports `duplication-only 0, three-distinct 3`, and its own comment states the rule — *"if fewer than three remain, duplication is the whole story and §3.2.3's inset is the fix. If three or more distinct polygons genuinely meet, it is not."* All three are genuine three-sheet junctions: 4 faces, 3 distinct triangles, 2 tetrahedra. The paper says the same thing in advance — *"Pairs of identical polygons … **define manifold connectivity**, but degenerate geometry"* — so §3.2.3 buys **intersection-freedom, not manifoldness**, and T-002 is the counter it should be graded against. The measurement that proves it: separating the two copies perfectly, by coning each from its own interior Steiner point, leaves the mesh closed (χ = 5, 1 component, 0 boundary edges) and `nm-edges` at **exactly 3**. **What this ticket needs before restarting:** (1) an acceptance criterion graded on T-002 self-intersections rather than on manifoldness; (2) a target on **`fbm_terrain`**, whose `duplication-only 2` of 4 bad edges is the whole reachable prize, rather than on `csg_difference`; (3) a home for the displacement, since the two the paper implies both tear here — a region is one face's share of a disk and its entire boundary is shared, so only the interior is free, and an interior-only inset does not reproduce §3.2.3's geometry. `csg_difference`'s `(3, 6, 6)` needs a different owner: it is a three-sheet junction, which is A-014's family but not §3.2.3's case.
