...
**Drafted as `P-177`–`P-226` / `R-182`–`R-231` on the `mac-air` checkout at `31ac0b8`, shifted by six to
`P-183`–`P-232` / `R-188`–`R-237` at the 2026-09-12 merge**, because `big`'s discovery loop had registered
`P-177`–`P-182` / `R-182`–`R-187` and landed four of them with their CSVs in the meantime. Landed numbers
never move; the arriving block did, as `M-440`'s phase did twice. `FINDINGS.md`'s Phase 29 numbering note
records the same shift, and nothing else about the rows changed.
...
`−7.33%`/`+2.27%`, read in full. `R-188` lands the instrument; it grades the four reference fields
`validate::accuracy` cannot (`✗107`), and `R-189`, `R-190`, `R-191`, `R-217`, `R-228`, `R-229` and
`R-235` use it.

**Four other things Phase 27 measured and this phase explains, bounds or replaces.** `✗116`'s
*"no filter can reach"* order 2 is a statement about the output representation, and Group D (`R-199`–
`R-202`) measures vertex order apart from surface order before spending `L` on curved patches. `M-472`'s
constant gap of 6.3–7.3× gets a three-factor decomposition (`R-233`). `M-459`'s *"roster is adversarial"*
becomes a designed experiment (`R-228`). `P-144`'s per-seed χ variance and `P-176`'s non-Gaussianity get a
Gaussian random field fixture with a closed-form `E[χ]`, `E[Area]` and `E[#critical points]` (`R-192`–
`R-195`, `R-234`).

**Order matters in nine places and nowhere else.** `R-188` runs before `R-191` and `R-227`. `R-192` runs
before `R-193`, `R-194`, `R-195` and `R-234`. `R-199` runs before `R-200`, `R-201` and `R-202`. `R-203`
runs before `R-204` and `R-205`. `R-209` runs before `R-210` and `R-211`. `R-223` runs before `R-224`.
`R-191` runs before `R-237`. Everything else is independent. **23 rows are `S`, 22 `M`, 5 `L`** (`R-192`,
`R-200`, `R-212`, `R-223`, `R-228`).

**Sixteen rows are expected to return nulls and that is registered rather than hoped:** `R-195`, `R-202`,
`R-208`, `R-211`, `R-214`, `R-215`, `R-218`, `R-220`, `R-222`, `R-224`, `R-227`, `R-230`, `R-231`,
`R-232`, `R-234`, `R-236`. Two more (`R-209`, `R-233`) are denominators whose most valuable outcome is
...
- **R-188 — P-183.** The case table already carries a ≤4% area estimator, and no field is ungradeable
- **R-189 — P-184.** The 6/26 split is an adjacency-pair choice, and the theorem says which pairs are consistent
- **R-190 — P-185.** Integrated mean curvature from the sign grid, as a third instrument against normal cycles
- **R-191 — P-186.** The error has a Wulff plot, and the plot is where BCC and cubic actually differ
- **R-192 — P-187.** A Gaussian random field fixture, and the closed-form χ that comes with it
- **R-193 — P-188.** Kac–Rice predicts the ambiguous-cell census from two numbers, before any mesh exists
- **R-194 — P-189.** The expected area of a Gaussian level set is `2√λ₂/π` per unit volume, and `M-13`'s constant should be it
- **R-195 — P-190.** A null registered on purpose — the value-noise percolation threshold is not the Gaussian one
- **R-196 — P-191.** Greedy meshing is finished only for axis-aligned planes, and digital plane recognition finishes the rest
- **R-197 — P-192.** The staircase has a period, and the period is a continued fraction
- **R-198 — P-193.** The normal estimator has a multigrid-convergence order, and it is the order DC's QEF inherits
- **R-199 — P-195.** Vertex error and surface error have different orders, and nobody has measured them apart
- **R-200 — P-194.** Curved PN triangles from the Hermite data DC already has, and the order they buy
- **R-201 — P-196.** The triquadratic's isosurface is a rational-quadratic patch per cell, and the table is tabulable
- **R-202 — P-197.** A null registered on purpose — Richardson extrapolation of vertex positions across two resolutions
- **R-203 — P-199.** The reach of an SDF is where its tube polynomial breaks, and that is a measurement not a lookup
- **R-204 — P-198.** `graph_cube_g5`'s hole at 9³ is a sampling-density event, and the reach predicts it
- **R-205 — P-200.** A topology certificate computed before extraction, from the reach and the grid spacing
- **R-206 — P-201.** The Besov index of the field predicts `M-474`'s `s`, from Haar coefficients alone
- **R-207 — P-202.** The word *fractal* pays its exponent — `fbm_terrain`'s triangle count scales as `h^{−(3−H)}`
- **R-208 — P-203.** A null registered on purpose — the octree's tree constraint costs a constant, not a rate
- **R-209 — P-204.** The case-index stream has an entropy rate, and no bit-packing beats it
- **R-210 — P-205.** Adjacent cells share four corners, so the case-index transition is a walk on a 16-out graph and its entropy is at most 4 bits
- **R-211 — P-206.** A null registered on purpose — the mesh is far above the metric-entropy floor of the surface class, and by how much
- **R-212 — P-207.** A sliding brush is a kinetic problem, and the number of certificate failures per frame is the incremental cost
- **R-213 — P-208.** Topology changes at discrete times, and the sealing check need only run at them
- **R-214 — P-209.** A null registered on purpose — sliced Wasserstein between mesh and surface measures, against Hausdorff
- **R-215 — P-210.** A null registered on purpose — displacement interpolation between LOD levels
- **R-216 — P-211.** A Huber QEF and a 2-means split on normals, against the `λ` the curl residual could not replace
- **R-217 — P-212.** Conformal prediction grades the four fields `validate::accuracy` cannot
- **R-218 — P-213.** A null registered on purpose — a W-class entanglement monotone on the cells where `|Δ|` is blind
- **R-219 — P-214.** Discrete Morse on the cubical complex is a third χ oracle, and its critical cells should be the body saddles
- **R-220 — P-215.** A null registered on purpose — a regular isovalue by Sard, located with the chemists' grid algorithm
- **R-221 — P-216.** A translating sphere's triangle count flickers with the sphere-problem exponent
- **R-222 — P-217.** A null registered on purpose — the gyroid's χ converges at integer cells-per-period and jitters otherwise, and that is Landau's rational/irrational dichotomy
- **R-223 — P-218.** Mesh one asymmetric unit of a symmetric field and replicate it, bit-identically, at `1/|G|` of the cost
- **R-224 — P-219.** A null registered on purpose — seams on mirror planes cannot crack
- **R-225 — P-220.** The shape derivative of area predicts a CSG edit's triangle delta before the remesh
- **R-226 — P-221.** A jittered grid kills the axis-aligned grain, and the Wulff plot says by how much
- **R-227 — P-222.** A null registered on purpose — dithering the isovalue does not unbias the mesh area
- **R-228 — P-223.** A space-filling roster of reference fields, so that no future mechanism is judged by a roster accident
- **R-229 — P-224.** The backward error of an extraction, and the condition number that turns it into Hausdorff
- **R-230 — P-225.** A null registered on purpose — extended revised affine arithmetic on `gyroid`'s rejected share
- **R-231 — P-226.** A null registered on purpose — solid and air have dual Betti numbers, and the split blocks are where the duality fails
- **R-232 — P-227.** A null registered on purpose — the flat norm sees a missing sheet that Hausdorff sees too, and costs an LP
- **R-233 — P-228.** `M-472`'s constant gap decomposes into a filter constant, a representation constant and a placement constant, and the ledger should say which is 5 of the 7
- **R-234 — P-229.** A null registered on purpose — ambiguous cells are Poisson at large correlation length
- **R-235 — P-230.** Area from the coarea integral and volume from the divergence theorem, as the tie-breaker for Group A
- **R-236 — P-231.** A null registered on purpose — eight Sobol points per coarse cell against eight corners
- **R-237 — P-232.** The mesh error's power spectrum peaks at the reciprocal lattice, and BCC moves the peaks
