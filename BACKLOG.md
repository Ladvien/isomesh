...
| | | **Add the `cd bevy_isomesh && cargo build --examples` job to `.github/workflows/ci.yml` in this same commit.** I-004 deliberately left it out rather than writing a step that skips itself when the directory is absent — a conditional that silently passes is exactly how `block-mesh`'s examples rotted to bevy 0.13. This line is the reminder; it is in the work queue rather than in anyone's head. | | |
