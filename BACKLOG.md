...
**86 tickets archived, 5 open.** Completed rows move to `BACKLOG_ARCHIVE.md` with their amendments
...
| | ID | Ticket | Size | Blocked by |
|---|---|---|---|---|
| ☐ | **GPU-010** | **GPU prefix scan and an indirect mesh draw — the change that actually removes the data movement.** M-149 breaks the 129³ GPU path into upload 8.63 ms, CPU prefix sum 3.27, counts read-back 1.97, geometry read-back 1.00. GPU-008b removes only the last. This removes the middle two: scan the per-cell counts **on the GPU**, write the total and the workgroup count into an indirect buffer, and draw with `draw_mesh_tasks_indirect` (present in `wgpu-29.0.4`, `api/render_pass.rs:303`) so neither the counts nor the total ever come home. **Acceptance:** a 129³ extraction that reads back **nothing**, measured against `gpu_vs_cpu.csv`'s committed row — and the scan asserted equal to the CPU prefix sum it replaces, element for element, because a scan that is subtly wrong produces a mesh that looks plausible. Note the output buffer must then be sized without knowing the total, so this also needs a decision about the allocation strategy (worst case, or a growable buffer with a re-dispatch). | L | GPU-008b |
