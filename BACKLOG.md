...
**125 tickets archived, 5 open.** Completed rows move to `BACKLOG_ARCHIVE.md` with their amendments
...
|---|---|---|---|
| ☐ | **E-213** | `marching_cubes_tunnel` — **the interior ambiguity made visible, and the tunnel meshed as a tunnel.** The third sibling of E-102 (an ambiguous *face*) and E-116 (the interior *decider*): this one shows the surface it decides about. One cell with six body saddles, the inner hexagon drawn as a closed ring of saddle points, and the two contours it separates — then the same cell under `InteriorAmbiguity::Ignore` beside it, two disks where there should be a cylinder. **The HUD must print the χ difference**, because that is the whole claim: −2 per tunnel and nothing otherwise (M-222). M-40 and M-208 between them say the field has to be `noise_cavity` or a hand-built cell — every other reference field renders two identical meshes. | S | A-002b |
