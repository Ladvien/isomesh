...
**185 tickets archived, 15 open.** Completed rows move to `BACKLOG_ARCHIVE.md` with their amendments
...
> BLOCKED: **on the public API shape, which is a design decision on a published crate.** The diagnosis is settled (✗22) and the work is understood; what is not decided is what the shipped surface looks like — export `SurfaceGate` as a public enum, add `MeshReport::satisfies(gate)`, or both. `isomesh` is live on crates.io at 0.0.5, so this is a permanent addition to the public API, and picking it unilaterally is exactly the class of choice `CLAUDE.md` reserves. **Note the size was understated**: `mod property` is `#[cfg(test)]`, so this promotes test-only scaffolding into the shipped crate rather than relaxing a visibility keyword.
