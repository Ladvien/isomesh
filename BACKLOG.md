...
**174 tickets archived, 7 open.** Completed rows move to `BACKLOG_ARCHIVE.md` with their amendments
...
| ☐ | **A-024** | **Surface Nets costs 2.6× more at exactly 128³ than at 127³ or 129³, and 128³ is the chunk size everybody uses.** A-023 removed the constant that was hiding it (M-286): 127³ and 129³ now cost **33.2 and 33.0** cycles per sample with 1.50 and 1.48 misses, and 128³ costs **84.3 with 4.72** — working sets 2% apart, a 2.6× penalty, reproducible across every run tonight and present on a field with **no surface at all**, so it is the scaffolding and not the geometry. The mechanism is not in doubt in outline: `n²·4 bytes` is exactly **64 KiB** at `n = 128`, which is a cache-set aliasing period on this machine, and `DualMesher` holds three dense arrays whose plane stride is that. **What is not known is whether it can be fixed cheaply**, and the honest options differ a lot in cost: **(a)** pad each plane by one cache line, which costs a multiply-add per index and some memory and is the standard remedy; **(b)** block the traversal so a plane is never walked end to end; **(c)** accept it and document `n ≠ 2ᵏ + 0` for chunk dimensions, which is free and moves the problem to the caller. **Acceptance:** `experiment_p12`'s 127/128/129 rows within 10% of each other, `family` and `experiment_p15` re-run, **golden hashes unchanged** — and if (a) is chosen, its cost at every *other* resolution measured and stated, because a fix that taxes the common case to help one grid size is not a fix. **Do not assume it reproduces elsewhere:** M-45 saw a 128³ spike on the Apple M5 too, but that machine's cache geometry is different and nothing has re-measured it since A-023 (M-005). | M | A-023 |
