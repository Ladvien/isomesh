...
| V-36 | **CoACD's intersection-free guarantee is a property of its *cut*, and its own default *merge* stage breaks it — which resolves a contradiction between two sources in this file's own corpus.** ✗20 recorded VisACD's figure that *"merging produces intersecting convex hulls in 35% of cases"* **about the hulls CoACD emits**. CoACD's own §6.2 says the opposite: cutting solid meshes with 3D planes *"results in flat boundaries between components. It ensures intersection-free convex hulls and avoids the defects caused by voxelization"*, and §6.4 repeats it — *"In this way, we ensure convex hulls of the decomposed components are intersection-free."* | A-026, both papers read directly. **Both are true, of different stages.** The recursive plane cut yields disjoint solids with flat boundaries, whose hulls cannot intersect. §6.5's post-process then *"traverse[s] all pairs of adjacent components and check[s] the Concavity of the merged component. If it's within the threshold ε, we will replace the two components with the merged one"* — and a merged pair's hull is the hull of their **union**, which can reach into a *third* neighbour. That is where the 35% comes from, and VisACD's sentence names the stage explicitly. **The consequence for A-026 is concrete and is not a compromise.** A cutting substrate needs cells that partition the interior; overlapping hulls double-count volume, which is the property that disqualified CPD (✗21). CoACD supplies exactly what is needed **with merging disabled** — and §6.5's merge exists solely to *"further reduce the number of components"*, so it is an economy lever, not a correctness one, the same shape as Delaunay-ness in T-024b. **It is on by default**: *"all of our methods include the merging stage unless otherwise noted."* So the knob that makes CoACD usable here is one you turn **off**, and nothing in its abstract would tell you that. |
