...
//!
//! Every row carries `cycles_per_sample` and `ghz`, and both are in the
//! registration's `records`, so they are promised rather than optional. They
//! come from `perf_event_open`, which is a Linux system call with no macOS
//! equivalent a bench can reach.
//!
//! On any other platform this **refuses and exits non-zero** rather than
//! reporting a run with those columns missing or invented. That is the shape
//! `experiment_p12` settles on for the same reason: a capability check that
//! fails loudly is one path choosing itself by measurement, where a silent
//! substitute would be two.
...
mod experiment {
    ...
    use crate::common::counters::Probe;
    ...
    /// Every arm, in the order the registration lists them: the two loop
    /// shapes in isolation, the extraction sweep over the shipped
    /// `sample_grid`, the golden fixture, then C1/C2/C3.
...
fn main() {
    ...
    {
        eprintln!(
            "{} records `cycles_per_sample` and `ghz`, which come from hardware\n\
             performance counters, and this platform has no `perf_event_open`.\n\
             Refusing rather than reporting a run whose registered columns were \
             never measured.\n\
             Run it on Linux; `perf_event_paranoid = 2` is permissive enough and \
             no root is needed.",
            prereg.id
        );
        std::process::exit(1);
