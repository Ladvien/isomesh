//! **P-35 — P-34 with its instrument floor removed and its corpus unmixed.**
//!
//! Ticket: R-034b. Pre-registered in the commit before this one; M-331 is the
//! falsification this corrects, with every correction derivable in advance:
//! the decision probed every iteration (the floor becomes 1/cold), the wall
//! grown to 20 × 24 for three digits of cold dynamic range, and tilts out of
//! the registered corpus — a gravity rotation moves every interface force
//! and is its own class, recorded beside the clauses rather than averaged
//! into them.
//!
//! ```bash
//! cargo bench --bench experiment_p35
//! ```
//!
//! Writes `docs/experiments/p-35.csv`.
//!
...
const COURSES: usize = 20;
const PER_COURSE: usize = 24;
const WB: f64 = 0.5;
const HB: f64 = 0.25;
...
fn solve_counted(proj: &AffineProjector, p: &Problem, start: &[f64]) -> (usize, bool, Vec<f64>) {
...
fn main() {
    ...
    let prereg = isomesh::experiment!("P-35");
    common::experiment::run(prereg, |run| {
        ...
        let base_proj = AffineProjector::new(&base_p);
        let zero = vec![0.0f64; base_p.cols];
        let (base_iters, base_ok, base_f) = solve_counted(&base_proj, &base_p, &zero);
        ...
        for e in 0..10 {
            ...
            let (ic, okc, _) = solve_counted(&proj, &p, &zero);
            let (iw, okw, _) = solve_counted(&proj, &p, &base_f);
            assert!(okc && okw, "weight edit failed to decide in the cap");
            let ratio = iw as f64 / ic as f64;
            nonsever.push(ratio);
            rows.push((format!("weight_{e}"), "local".into(), ic, iw, ratio, true));
        }
        for (e, deg) in [0.3f64, -0.3, 0.5, -0.5, 0.7].iter().enumerate() {
            ...
            let ratio = iw as f64 / ic as f64;
            // Recorded class only — a tilt re-loads every interface (M-331).
            rows.push((
                format!("tilt_{e}"),
                "tilt_recorded".into(),
                ic,
        ...
        let eligible: Vec<usize> = blocks
            .iter()
            .enumerate()
            .filter(|(_, b)| (4..=14).contains(&b.course) && b.x0 > 1.0 && b.x1 < 11.0)
            .map(|(i, _)| i)
            .collect();
        ...
        for (e, &gone) in picked.iter().enumerate() {
            ...
            let proj = AffineProjector::new(&p);
            let (ic, okc, _) = solve_counted(&proj, &p, &vec![0.0; p.cols]);
            let (iw, okw, _) = solve_counted(&proj, &p, &warm);
            if !(okc && okw) {
                collapsed += 1;
                rows.push((
                    format!("remove_{e}"),
        ...
        println!(
            "C1 (local edits cheap): median ratio {m_non:.3} -- {} (H says <= 0.15; floor now 1/cold)",
