//! **P-30 — P-29's bimodality question, re-asked with the mode-guarded statistic.**
//!
//! Ticket: R-031. Pre-registered in the commit before this one; the
//! falsified first attempt is M-326, and its instrument defect — a 1% trim
//! that eats a 63-edge winner mode — is what this run removes. Same model,
//! same seeds, same constants, same arms as `experiment_p29.rs`; only the
//! reader changes, plus a recorded water-flux share.
//!
//! ```bash
//! cargo bench --bench experiment_p30
//! ```
//!
//! Writes `docs/experiments/p-30.csv`.
//!
...
/// Max qualifying gap in sorted ln(mean aperture), NO trim: a gap counts
/// only when both sides of the split hold at least `MODE_GUARD` values —
/// eight-fold below the smallest possible winner mode (one input→output
/// chain, 63 edges) and eight-fold above single-edge outliers.
const MODE_GUARD: usize = 8;

fn guarded_gap(values: &mut [f64]) -> f64 {
    values.sort_unstable_by(f64::total_cmp);
    let n = values.len();
    let mut best = 0.0f64;
    for i in MODE_GUARD..=n.saturating_sub(MODE_GUARD) {
        if i == 0 || i >= n {
            continue;
        }
        best = best.max(values[i] - values[i - 1]);
    }
    best
}

/// Max consecutive gap in sorted ln(mean aperture), 1% tails dropped — the
/// P-29 statistic, kept as a recorded column for continuity with M-326.
...
fn run_arm(
    ...
    let mut q_sorted = last_q.clone();
    q_sorted.sort_unstable_by(|x, y| y.total_cmp(x));
    let q_total: f64 = q_sorted.iter().sum();
    let water_share: f64 = if q_total > 0.0 {
        q_sorted[..top].iter().sum::<f64>() / q_total
    } else {
        0.0
    };
    ...
    ArmResult {
        ...
        flux_water_top10_pct: water_share * 100.0,
...
fn main() {
    ...
    let prereg = isomesh::experiment!("P-30");
    common::experiment::run(prereg, |run| {
        ...
        let g0 = guarded_gap(&mut t0_sorted);
        assert!(
            g0 < GAP_THRESHOLD,
        ...
        let gs = guarded_gap(&mut shifted);
        assert!(
            gs >= GAP_THRESHOLD,
            "detector red failed: synthetic +4σ half-shift gap {gs} < {GAP_THRESHOLD}"
        );
        println!(
            "detector (guarded): t0 gap {g0:.5} (< {GAP_THRESHOLD} — green), synthetic gap {gs:.3} (≥ {GAP_THRESHOLD} — red demonstrated)"
        );
        ...
        assert!(
            (ctl_result.guarded_gap_ln >= GAP_THRESHOLD)
                == (het_result.guarded_gap_ln >= GAP_THRESHOLD),
            "dt control changed the guarded bimodal verdict: {} vs {}",
            ctl_result.guarded_gap_ln,
            het_result.guarded_gap_ln
        );
        assert!(
        ...
        for (name, cap, r) in [
            ...
            let bimodal = r.guarded_gap_ln >= GAP_THRESHOLD;
            let bt = r
                .breakthrough_years
                .map_or_else(|| "none".to_string(), |t| format!("{t:.1}"));
            println!(
                "{:>16} {:>6} {:>9.1} {:>12} {:>9.3} {:>9.3} {:>8} {:>11.2} {:>11.2} {:>9.4} {:>8.2}",
                name,
                r.ticks,
                r.years,
                bt,
            ...
            run.record(&[
                ...
                ("guarded_gap_ln", format!("{:.4}", r.guarded_gap_ln)),
                ("guarded_bimodal", bimodal.to_string()),
                ("flux_top10_pct", format!("{:.2}", r.flux_top10_pct)),
                (
                    "flux_water_top10_pct",
                    format!("{:.2}", r.flux_water_top10_pct),
                ),
        ...
        println!();
        let c1 = hom_result.guarded_gap_ln >= GAP_THRESHOLD
            && het_result.guarded_gap_ln >= GAP_THRESHOLD
            && rl_result.guarded_gap_ln < GAP_THRESHOLD;
        println!(
            "C1 (guarded bimodal split): hom {:.3} / het {:.3}, recharge {:.3} -- {}",
            hom_result.guarded_gap_ln,
            het_result.guarded_gap_ln,
            rl_result.guarded_gap_ln,
            if c1 { "HELD" } else { "FALSIFIED" }
        );
        println!(
            "recorded: dissolution top-10% {:.1}% (hom) / {:.1}% (het); water top-10% {:.1}% / {:.1}% (M-326 closed C2 at 78%)",
            hom_result.flux_top10_pct,
            het_result.flux_top10_pct,
            hom_result.flux_water_top10_pct,
            het_result.flux_water_top10_pct
        );
        println!(
