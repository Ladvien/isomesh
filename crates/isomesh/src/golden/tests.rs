...
/// Guard against the sweep silently shrinking. 7 fields x 3 algorithms x 3
/// resolutions.
#[test]
fn every_combination_is_covered() {
    let entries = compute_all();
    // 7 fields x 4 algorithm rows x 3 resolutions. `mc33` is a row rather than
    // an algorithm — Marching Cubes with the asymptotic decider switched on.
    assert_eq!(entries.len(), 7 * 4 * 3, "{} combinations", entries.len());
