...
/// Guard against the sweep silently shrinking. 14 fields x 9 algorithm rows x 3
/// resolutions.
#[test]
fn every_combination_is_covered() {
    let entries = compute_all();
    // 14 fields x 9 algorithm rows x 3 resolutions. `marching_cubes+decider` is a
    ...
    assert_eq!(entries.len(), 14 * 9 * 3, "{} combinations", entries.len());
